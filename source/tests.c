    ...
    const gnutls_pubkey_t sender_public_key)
{
    ...
    ret = openssl_hpke_encap_and_seal_auth(
        recipient_public_key_raw, recipient_public_key_raw_len,
        sender_private_key, OSSL_HPKE_KEM_ID_X25519,
        OSSL_HPKE_KDF_ID_HKDF_SHA256, OSSL_HPKE_AEAD_ID_CHACHA_POLY1305, info,
        sizeof(info) - 1, aad, sizeof(aad) - 1, pt, sizeof(pt) - 1, &enc,
        &enclen, &ct, &ctlen, exp, explen);
    if (ret != 1)
    {
    ...
    const gnutls_pubkey_t sender_public_key)
{
    ...
    ret = openssl_hpke_encap_and_seal_psk_auth(
        recipient_public_key_raw, recipient_public_key_raw_len,
        sender_private_key, OSSL_HPKE_KEM_ID_X25519,
        OSSL_HPKE_KDF_ID_HKDF_SHA256, OSSL_HPKE_AEAD_ID_CHACHA_POLY1305, psk,
        sizeof(psk), psk_id, info, sizeof(info) - 1, aad, sizeof(aad) - 1, pt,
